...
public:
    ...
    void move(int fromIndex, int toIndex) {
        if (fromIndex <= 0 || toIndex <= 0) {
            cout << "Invalid indices provided.\n";
            return;
        }

        ListNode* currNode = head;
        int count = 1;
        while (currNode != nullptr && count < fromIndex) {
            currNode = currNode->nextNode;
            count++;
        }

        if (currNode == nullptr) {
            cout << "Source index is out of range.\n";
            return;
        }

        ListNode* nodeToMove = currNode;
        if (nodeToMove == head) {
            head = nodeToMove->nextNode;
            if (head) head->prevNode = nullptr;
        } else if (nodeToMove == tail) {
            tail = nodeToMove->prevNode;
            if (tail) tail->nextNode = nullptr;
        } else {
        ...
        currNode = head;
        count = 1;
        while (currNode != nullptr && count < toIndex) {
            currNode = currNode->nextNode;
            count++;
        }

        if (currNode == nullptr) {
            append(nodeToMove->val);
            delete nodeToMove;
        } else {
            nodeToMove->nextNode = currNode;
            nodeToMove->prevNode = currNode->prevNode;
            if (currNode->prevNode) {
                currNode->prevNode->nextNode = nodeToMove;
            } else {
                head = nodeToMove;
            }
            currNode->prevNode = nodeToMove;
        }

        cout << "Node moved successfully.\n";
    }
    ...
    void duplicate(int index) {
        ...
        ListNode* currNode = head;
        int count = 1;
        while (currNode != nullptr && count < index) {
            currNode = currNode->nextNode;
            count++;
        }
        ...
        append(currNode->val);
        cout << "Node duplicated successfully.\n";
    }
};
